/*
Super Canon Remote firmware by Kevin Osborn
...
extern const struct powercode CanonCode PROGMEM;
...
int main(void) {
  ...
  for (i=0; i<1; i++) {   // repeat the code twice
    ...
    uint8_t freq = pgm_read_byte(&CanonCode);
    ...
    do {
      ...
      ontime = pgm_read_word(&CanonCode+(j*4)+1);
      offtime = pgm_read_word(&CanonCode+(j*4)+3);
