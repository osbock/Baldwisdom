/*
LaserTag Team ops for TV-b-gone
...
extern const struct powercode shotCode PROGMEM;
...
int main(void) {
  ...
  for (i=0; i<1; i++) {   // repeat the code twice
    ...
    uint8_t freq = pgm_read_byte(&shotCode);
    ...
    do {
      ...
      ontime = pgm_read_word(&shotCode+(j*4)+1);
      offtime = pgm_read_word(&shotCode+(j*4)+3);
